...
class MessageController extends AbstractController {
    ...
    public function index(Request $request, EntityManagerInterface $manager): Response {
        ...
        if ($messageForm->isSubmitted() && $messageForm->isValid()) {
            ...
            $config = array(
                'clientId' => "8967Ksh6WEscJuMZxy7Teh0TvdCoF7UH",
                'clientSecret' => "OOvb99hlvIwO5Guq"
            );

            $osms = new Osms($config);
            $osms->setVerifyPeerSSL(true);
            $response = $osms->getTokenFromConsumerKey();

            $config1 = array('token' => $response['access_token']);
            $osms1 = new Osms($config1);


            $osms1->setVerifyPeerSSL(true);
            foreach ($clients as $cli) {
                $phone = $cli->getTelephone(); 
                $response1 = $osms1->sendSms(
                        $sender = 'tel:+221773831219', $receiver = "tel:+221$phone", $message1 = $message->getMotif() , "$sender", "$receiver", "$message1"
                );
            }
        ...
        return $this->render('smarketing/home.html.twig', [
...
class Osms {
     ...
     * @var string
     */
    protected $clientId = '8967Ksh6WEscJuMZxy7Teh0TvdCoF7UH';
     ...
     * @var string
     */
    protected $clientSecret = 'OOvb99hlvIwO5Guq';
    ...
    $senderAddress, $receiverAddress, $message, $senderName = ''
    ) {
    ...
    $headers, $args, $url, $method, $successCode, $jsonEncodeArgs = false
    ) {
        $ch = curl_init();
